...
class Plane(object):
    ...
    def __eq__(self, plane):
        ...
        if not self.is_parallel_with(plane):
            return False

        basepoint_diffrence = self.basepoint.minus(plane.basepoint)
        return self.normal_vector.is_orthogonal_with(basepoint_diffrence)

    def is_parallel_with(self, plane):
        return self.normal_vector.is_parallel_with(plane.normal_vector)
